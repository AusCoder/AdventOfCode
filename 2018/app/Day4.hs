      ...
      go acc start (x : xs) =
        let evMin = eventMinute x
         in case eventType x of
              ...
              GuardWakesUp ->
                let foldFn x m = Map.insert x ((+) 1 . fromMaybe 0 . Map.lookup x $ m) m
...
day4a = runDay "input/day4.txt" parserGuardEvents (Right . part1)

part2 = doPart (comparing $ \(_, cs) -> if Map.size cs == 0 then -1 else maximum cs)

day4b = runDay "input/day4.txt" parserGuardEvents (Right . part2)
